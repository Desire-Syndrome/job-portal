...
const Header = () => {
	...
	const closePopup = () => {
		...
		setPopupState("Login"); setShowCompanyPopup(false); setShowUserPopup(false);
	}
	...
	return (<>
		...
		<div className='shadow py-4'><div className='container px-4 2xl:px-20 mx-auto flex flex-wrap justify-between items-center lg:order-1'>
			{/* Logo */}
			...
			<div className='flex gap-2 md:text-sm text-xs lg:order-3'>
				{company ? (<CompanyMenu />
					...
					<button onClick={() => setShowCompanyPopup(true)}
		...
		{showCompanyPopup && (
			<div className="fixed inset-0 bg-white bg-opacity-70 flex items-center justify-center z-100"><div onClick={closePopup} className='fixed inset-0 z-150'></div>
				<div className="bg-white rounded-lg shadow p-6 w-80 lg:w-96 relative border-blue-600 border-opacity-70 border-2 z-200">
					...
					<h2 className="text-lg text-center font-semibold mb-4">Company {popupState}</h2>
						...
						{popupState === 'Registration' && (<>
							<div className='text-center my-6 w-100 py-2 bg-slate-200 rounded-lg'>
								<label htmlFor="image">
									...
									<p className='mt-2'>Upload company logo</p>
								</label>
							</div>
							<div className='border border-gray-300 px-4 py-2 flex items-center gap-2 rounded-full mt-2'>
								...
								<input onChange={e => setName(e.target.value)}
									value={name} type="text" placeholder='Company name' className='no-focus text-sm border-none' required />
							</div>
						</>)}
						<div className='border border-gray-300 px-4 py-2 flex items-center gap-2 rounded-full mt-2'>
					...
					<form onSubmit={userLogin}>
						{popupState === 'Registration' && (<>
							...
							<div className='border border-gray-300 px-4 py-2 flex items-center gap-2 rounded-full mt-2'>
								<img src={assetsImages.person_icon} className='h-4 w-4' alt="person icon" />
								<input onChange={e => setName(e.target.value)}
									value={name} type="text" placeholder='Name' className='no-focus text-sm border-none' required />
							</div>
						</>)}
						<div className='border border-gray-300 px-4 py-2 flex items-center gap-2 rounded-full mt-2'>
							<img src={assetsImages.email_icon} className='h-4 w-4' alt="email icon" />
