...
const UserInfo = () => {
	...
	const [resume, setResume] = useState(user.image);
			...
			<div className="flex gap-2 mb-3 mt-3">
				{isResumeEdit || user && user.resume === "" ? (<>
					<label className='flex items-center' htmlFor="resume-upload">
						<p className='bg-blue-100 text-blue-600 px-4 py-2 rounded-lg cursor-pointer'>{resume?.name || "Select Resume"}</p>
					...
					<div className="flex items-center ">
						<a href="#" className="bg-blue-100 text-blue-600 px-4 py-2 rounded-lg mr-2 cursor-pointer">{resume?.name || "No Resume"}</a>
						<button onClick={() => setIsResumeEdit(true)}
